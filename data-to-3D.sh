...
then
	...
	then
		...
		echo
		ACTION=help-error
	elif ! [ "$(find "$WORKSPACE" -mindepth 1)" = "" ] && [ "$SKIP_COLMAP" = 0 ]
	then
	...
	then
		if ! [ "$WORKSPACE_SET" = 1 ]
		then
			echo ERROR: cannot use the sparse and dense point cloud from a previous run the workspace directory of that run isn\'t specified
			echo
			ACTION=help-error
		else
			if ! [ -e "$WORKSPACE/colmap/dense/0/fused.ply" ]
			then
				echo ERROR: There isn\'t at least one point cloud generated by colmap to skip it.
				echo
				ACTION=help-error
			fi
			if ! [ -e "$WORKSPACE/log" ]
			then
				echo ERROR: There isn\'t a log file in the workspace dir from the previous run
				echo
				ACTION=help-error
			fi
			if [ -e "$WORKSPACE/openmvs" ]
			then
				echo ERROR: There exists an openmvs directory in the workspace.
				echo
				ACTION=help-error
			fi
		fi
...
'                            still unstable\n'\
' --skip-colmap -s          Skip running the colmap step and use any existing\n'\
'                            data in the workspace directory from a previous run\n'\
...
if [ "$SKIP_COLMAP" = 1 ]
then
	SEED=$(sed '/seed/s/.*seed //p;2q' -n < "$WORKSPACE"/log)
	if [ "$SEED" = "" ]
	then
		echo WARNING: failed to find previous seed on log file
	fi
fi
if [ "$SEED" = "" ]
then
